    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="container mx-auto px-4 pt-28 space-y-12">
        ...
        <motion.div initial="hidden" animate="visible" variants={containerVariants} className="flex flex-col md:flex-row justify-between items-end gap-4">
          <div>
            <motion.h1 variants={itemVariants} className="text-4xl md:text-5xl font-black text-white tracking-tight leading-none">
              Welcome, <span className="text-primary text-glow">{user?.name}</span>
            </motion.h1>
            <motion.p variants={itemVariants} className="text-white/40 mt-3 font-light text-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 h-full">
          ...
          <div className="md:col-span-8 grid grid-cols-2 lg:grid-cols-4 gap-4">
            <motion.div variants={itemVariants} className="glass-card p-6 rounded-3xl flex flex-col justify-between">
              ...
              <div>
                <div className="text-3xl font-black text-white leading-none mb-1">{stats.totalQuizzes}</div>
                <div className="text-[10px] uppercase font-black tracking-widest text-white/30">Total Quizzes</div>
              </div>
            </motion.div>
            <motion.div variants={itemVariants} className="glass-card p-6 rounded-3xl flex flex-col justify-between">
              <span className="text-3xl">📊</span>
              <div>
                <div className="text-3xl font-black text-white leading-none mb-1">{stats.avgScore}%</div>
                <div className="text-[10px] uppercase font-black tracking-widest text-white/30">Avg. Accuracy</div>
              </div>
            </motion.div>
            <motion.div variants={itemVariants} className="glass-card p-6 rounded-3xl flex flex-col justify-between">
              <span className="text-3xl">🏆</span>
              <div>
                <div className="text-3xl font-black text-white leading-none mb-1">{stats.bestScore}%</div>
                <div className="text-[10px] uppercase font-black tracking-widest text-white/30">Best Score</div>
              </div>
            </motion.div>
            <motion.div variants={itemVariants} className="glass-card p-6 rounded-3xl flex flex-col justify-between overflow-hidden">
              <span className="text-3xl">⭐</span>
              <div>
                <div className="text-xl font-black text-white leading-tight mb-1 truncate">{stats.bestCategory}</div>
                <div className="text-[10px] uppercase font-black tracking-widest text-white/30">Top Subject</div>
              </div>
            </motion.div>
          </div>
          ...
          <motion.div variants={itemVariants} className="md:col-span-4 glass-card p-6 rounded-3xl relative overflow-hidden flex flex-col justify-center min-h-[160px] group">
            ...
            <motion.p 
              ...
              className="text-white/80 text-lg font-medium italic leading-relaxed relative z-10"
            >
              "{currentJoke}"
            </motion.p>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="md:col-span-4 glass-card rounded-3xl flex flex-col h-[500px] order-2 md:order-1">
            <div className="p-6 border-b border-white/5 flex justify-between items-center">
              ...
              <span className="text-[10px] text-white/30 font-bold">{isConnected ? 'ONLINE' : 'CONNECTING...'}</span>
            </div>
            <div className="flex-1 p-6 overflow-y-auto space-y-4 flex flex-col-reverse custom-scrollbar">
               <div className="flex flex-col space-y-4">
                  {chatHistory.length === 0 ? (
                    <div className="m-auto text-sm text-white/20 italic font-light">No active transmissions...</div>
                  ) : (
                    chatHistory.map((msg) => (
                      <div key={msg.id} className="group">
                        ...
                        <div className="bg-white/5 border border-white/5 px-4 py-2.5 rounded-2xl rounded-tl-none text-sm text-white/70 leading-relaxed">
                          {msg.text}
                        </div>
                      </div>
                    ))
            ...
            <form onSubmit={sendChatMessage} className="p-4 border-t border-white/5 bg-black/20 rounded-b-3xl">
              <div className="relative flex items-center">
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-3 text-sm focus:outline-none focus:border-primary/40 focus:bg-white/10 transition-all text-white placeholder:text-white/20 shadow-inner"
                />
                <button 
                  type="submit" 
                  disabled={!chatMessage.trim() || !isConnected}
                  className="absolute right-2 bg-primary text-white p-2 px-4 rounded-xl hover:bg-primary/80 disabled:opacity-30 transition-all font-bold text-xs"
                >
                  SEND
                </button>
              </div>
          ...
          <motion.div variants={itemVariants} className="md:col-span-8 space-y-6 order-1 md:order-2">
            <div className="flex flex-col sm:flex-row justify-between items-end gap-4">
              <h2 className="text-3xl font-black text-white tracking-tight">Available Forging Tracks</h2>
              <div className="relative w-full sm:w-80">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-3 bg-white/5 border border-white/5 rounded-2xl text-sm focus:outline-none focus:border-primary/30 transition-all text-white placeholder:text-white/20 font-medium"
                />
              </div>
            </div>
              ...
              {cats.filter(c => c.name.toLowerCase().includes(searchQuery.toLowerCase())).map(cat => (
                <motion.div key={cat.id} whileHover={{ y: -8 }} transition={{ type: "spring", stiffness: 400 }}>
                  ...
                  >
                    ...
                    />
                    <h3 className="text-2xl font-black text-white mb-2 leading-tight group-hover:text-primary transition-colors">{cat.name}</h3>
                    <p className="text-white/40 text-sm font-light leading-relaxed line-clamp-2">{cat.description}</p>
                    <div className="mt-6 flex items-center justify-between">
                       ...
                       <span className="text-[10px] font-black uppercase text-white/20 group-hover:text-primary transition-all">Start Track →</span>
                    </div>
                  </Link>
                </motion.div>
              ))}
        ...
        {history.length > 0 && (
          <motion.div variants={itemVariants} className="pt-6">
            <h2 className="text-3xl font-black text-white tracking-tight mb-8">Archived Chronicles</h2>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-white/5 border-b border-white/5">
                      ...
                      <th className="p-6 text-[10px] font-black uppercase tracking-widest text-white/30">Performance</th>
                      <th className="p-6 text-[10px] font-black uppercase tracking-widest text-white/30 hidden sm:table-cell">Metrics</th>
                      <th className="p-6 text-[10px] font-black uppercase tracking-widest text-white/30 hidden md:table-cell">Duration</th>
                      <th className="p-6 text-[10px] font-black uppercase tracking-widest text-white/30">Timestamp</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/5">
                    {history.slice(0, 8).map(a => (
                      <tr key={a.id} className="hover:bg-white/[0.02] transition-colors group">
                        <td className="p-6 font-bold text-white group-hover:text-primary transition-colors">{getCategoryName(a.categoryId)}</td>
                        <td className="p-6">
                          <div className="flex items-center gap-3">
                            <span className="font-black text-xl text-glow">{Math.round((a.correctAnswers/a.totalQuestions)*100)}%</span>
                            <div className="h-1.5 w-16 bg-white/5 rounded-full overflow-hidden hidden sm:block">
                              <div className="h-full bg-primary" style={{ width: `${(a.correctAnswers/a.totalQuestions)*100}%` }} />
                            </div>
                          </div>
                        </td>
                        <td className="p-6 text-white/40 font-light hidden sm:table-cell">{a.correctAnswers} / {a.totalQuestions} Hits</td>
                        <td className="p-6 text-white/40 font-light hidden md:table-cell">{Math.floor(a.timeTaken / 60)}m {a.timeTaken % 60}s</td>
                        <td className="p-6 text-white/20 font-bold text-xs">{new Date(a.submittedAt).toLocaleDateString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
